from typing import List

import feapder
from wyy_setting import base_url, music_api
from utils.node import get_node
ctx=get_node(js_dir='./js', js_path='main.js')


class MusicSpider(feapder.AirSpider):
    def __init__(self,ids:List[int]):
        super().__init__()
        self.ids = ids
    def get_encrypt_data(self):
        encrypt_data = {
            "csrf_token": "41cc3a81b7f9763f65a52a7bcba0cc86",
            "encodeType": "aac",
            "ids":str(self.ids),
            "level":"exhigh"
        }
        print(encrypt_data)
        result = ctx.call('get_params', encrypt_data)
        params_list = result['return_value']
        return {
            'params': params_list[0],
            'encSecKey': params_list[1]
        }


    def start_requests(self):
        encrypt_data=self.get_encrypt_data()
        yield feapder.Request(base_url + music_api,
                              callback=self.parse,
                              data=encrypt_data
                            )

    def download_midware(self, request):
        request.headers = {
            'Cookie': 'MUSIC_U=;',
        }
        return request

    def parse(self, request, response):
        print(response.json)
