import setting
import requests
from utils import  get_node
ctx=get_node(js_dir='js', js_path='min.js')
def get_time(dt:int):
    seconds = dt // 1000  # 整秒
    minutes, sec = divmod(seconds, 60)
    return f'{int(minutes)}:{int(sec)}'
list_url='https://music.163.com/weapi/cloudsearch/get/web?csrf_token=41cc3a81b7f9763f65a52a7bcba0cc86'
encrypt_data = {
    'hlpretag': '<span class="s-fc7">',
    'hlposttag': '</span>',
    's': '他不懂',
    'type': '1',
    'offset': '0',
    'total': 'true',
    'limit': '30',
    'csrf_token': '41cc3a81b7f9763f65a52a7bcba0cc86'
}
result = ctx.call('get_params', encrypt_data)
params_list=result['return_value']
form = {
    'params': params_list[0],
    'encSecKey': params_list[1]
}
headers={
    'Cookie':'MUSIC_U=;',
}
print(form)
response = requests.post(url=list_url, data=form,headers=headers)
json_data=response.json()
songs=json_data['result']['songs']
for song in songs:
    song_name=song['name']
    song_id=song['id']
    artists_url=setting.base_url+setting.artist+str(song['ar'][0]['id'])
    artists_name=song['ar'][0]['name']
    album=setting.base_url+setting.album+str(song['al']['id'])
    album_name=song['al']['name']
    dt=get_time(int(song['dt']))
    print(f'{song_name}== {artists_name}== {album_name} =={dt}')

